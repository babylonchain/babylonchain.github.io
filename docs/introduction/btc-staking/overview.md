...
2. **Validation on PoS Chain:** Once the staking transaction is confirmed on the Bitcoin chain, the staker (or validator the staker delegates to) can start validating transactions on the PoS chain by signing blocks using the EOTS secret key. During her validation duty, there are two possible paths:
   - **Happy Path:** In the honest scenario, the staker follows the protocol and earns yield. The staker can then unbond via two approaches: 1) waiting until the timelock in the UTXO has been expired; or 2) submitting an unbonding transaction on Bitcoin. It unlocks the bitcoin and returns it to her after a parameterized unbonding period.
